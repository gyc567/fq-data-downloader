...
async fn verifies_when_amount_covers_required() {
    ...
    let challenge = m.prepare_challenge("qt_1", "0.050000");
    let p = proof("qt_1", "0.050000");
    let r = m.verify(&challenge, &p).await.unwrap();
    match r {
...
async fn overpayment_also_verifies() {
    ...
    assert!(m.verify(&challenge, &p).await.is_ok());
}
...
async fn underpayment_is_rejected() {
    ...
    let err = m.verify(&challenge, &p).await.unwrap_err();
...
async fn expired_quote_is_rejected() {
    let m = MockFacilitator::new();
    let mut challenge = m.prepare_challenge("qt_1", "0.050000");
    // Backdate expiry to 1 hour ago.
...
async fn payment_required_serializes_to_canonical_shape() {
    let m = MockFacilitator::new();
    let challenge = m.prepare_challenge("qt_x", "1.000000");
